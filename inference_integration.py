...
def integration(reviews):
    reviews = "\n".join([
            f"review_no:{review['id']}\tcontent:{review['document']}"for review in reviews
        ])
    print(reviews)
...
#     promptTemplate
    ...
    out_put = StrOutputParser()

    chain = (prompt_lachain | model | out_put)
